...
_PeerUpdateAction = namedtuple('Action', 'name type peer')
...
class Config:
    ...
    interface = property(
        __get_interface, __set_interface, __del_interface,
    ...
    def create_interface(
        ...
        address:        address.type = DEFAULT_PRIVATE_ADDRESS,
        private_key:    Optional[private_key.type] = None,
        ...
        interface = Interface.create(
            ...
            address=normalize_address(address), private_key=private_key,
            listen_port=listen_port, dns=dns, table=table, mtu=mtu,
            pre_up=pre_up, post_up=post_up, pre_down=pre_down,
            post_down=post_down, save_config=save_config,
        )
    ...
    def update_interface(self, **props: PropValues) -> None:
        if interface := self.interface:
    ...
    def _process_peer_updates(
        ...
        peers_by_public_key = {
            p.public_key: p
            for p in peers
            if p.public_key is not None
        }
        
        Action = namedtuple('Action', 'name type peer')
        actions = []
        ...
        for name, update in updates.items():
            ...
            if pubkey := update.get('public_key'):
                if by_pubkey := peers_by_public_key.get(pubkey):
                    if peer is None:
                        peer = by_pubkey
    ...
    def _resolve_client_keys(
        ...
        if (
            private_key is not None and 
            public_key is not None and
            pubkey(private_key, self.wg_bin_path) != public_key
        ):
            raise ValueError(
    ...
    def add_client(
        ...
        public_key: Optional[Peer.private_key.type] = None,
    ) -> Optional[Config]:
        interface = self.interface
        
        if interface is None:
            raise Exception(f"No Interface - add one before adding clients")
        ...
        self.add_peer(
            name=name, description=description, allowed_ips=private_address,
        ...
        return self._make_client_config(
            name=name, private_address=private_address,
            allowed_ips=allowed_ips, preshared_key=preshared_key, dns=dns,
            persistent_keepalive=persistent_keepalive, private_key=private_key,
        )
    ...
    def _make_client_config(
        ...
        client_config = Config(
            hostname=name, name=None, dir=None, wg_bin_path=self.wg_bin_path,
        )
        client_config.create_interface(
            private_key=private_key, address=private_address, dns=dns,
            description=(
        ...
        client_config.add_peer(
            name=f"{self.name}@{self.hostname}", allowed_ips=allowed_ips,
    ...
    def _modify_client(self, peer, update) -> Optional[Config]:        
        ...
        if not peer.has_changes(**peer_props):
            ...
            return self._make_client_config(
                ...
                **pick(update, ('private_key', 'allowed_ips', 'preshared_key',
                    'dns', 'persistent_keepalive'))
            )
        ...
        return self._make_client_config(
            ...
            **pick(update, ('allowed_ips', 'preshared_key', 'dns',
                'persistent_keepalive'))
        )
        
    def update_clients(
        self,
        updates: Dict[name, PropValues],
    ) -> Dict[str, Config]:
        client_configs = {}
    ...
    def write(
        self,
        dest: Union[TextIO, Path, str, None] = None,
        *args,
        **kwds
    ):
        if dest is None:
